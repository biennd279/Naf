...
Dù rằng ZAP đã cung cấp một Concept tương đối hoàn chỉnh để thực hiện được
việc kiểm thử an toàn thông tin, nhưng phần lớn các ZAP Addons lại rất
...
thử an ninh cần sử dụng trực tiếp trên Web Browser một thời lượng lớn
dù rằng các ZAP Addons có thể thay thế một phần việc này.

Một vấn đề khác là do các ZAP Addons được thiết kế để chạy được Stand-Alone
nên sự tương tác giữa các ZAP Addons chưa cao dù rằng chỉ với một số Addons
Core đã có thể hoàn thành lượng lớn công việc đánh giá lỗ hổng.
...
Ngoài ra, các ZAP Addons cũng không thể cung cấp được các công cụ quét
...
Addons để nâng mức tự động lên mức độ chấp nhận được và bổ sung thêm
...
một đầu ra nhưng một trình pseudo-shell.
...
của lệnh sẽ được gửi về bằng cách phân tích phản hồi của yêu cầu.
...
xem dữ liệu trả về thông qua việc quan sát phản hồi.
